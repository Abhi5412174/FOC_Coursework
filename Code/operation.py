...
def generate_invoice(transactions):
    ...
    '''
    filename = f'order_invoice_of_{transactions[0]["employee_name"]}_{datetime.datetime.now().strftime("%Y%m%d%H%M%S")}.txt'
    furniture_ids = []
    manufacturers = []
    product_names = []
    quantities = []
...
def sell_furniture(customer_name, transactions):
    ...
    if valid_transactions:
        ...
        filename = f'sale_invoice_of_{customer_name}_{datetime.datetime.now().strftime("%Y%m%d%H%M%S")}.txt'
